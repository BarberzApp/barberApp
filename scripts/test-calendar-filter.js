...
async function testCalendarFilter() {
  try {
    console.log('🔍 Testing calendar filter with payment_status = succeeded...');
    console.log('==============================================');
    
    // Test what the calendar should show now
    const { data: successfulBookings, error } = await supabase
      .from('bookings')
      .select('*')
    ...
    const { data: testBarberBookings, error: barberError } = await supabase
      .from('bookings')
      .select('*')
